...
# def chase(start, rect_list, player_x, player_y):
		# surface = window
		# colour = BLACK
		# start_line = [int(start[0] + (char_width / 2)), int(start[1] + (char_height / 2))]
		# end_line = [int(player_x + (char_width / 2)), int(player_y + (char_height / 2))]
		# end = [player_x, player_y]
		# width = 1
		# line = pygame.draw.line(surface, colour, start_line, end_line, width)
		# sighted = line.collidelist(rect_list) == -1
		# new_start = [o_rect[0], o_rect[1]]
		# if sighted == True:
			# new_start[0] = end[0] + (start[0] - end[0]) - 1 
			# new_start[1] = end[1] + (start[1] - end[1]) - 1 
		# return new_start
...
if __name__ == "__main__":
	...
	while run_game:
		...
		pressed_keys = pygame.key.get_pressed() # Check what key is being pressed in the current iteration of the game loop
		mouse_pos = pygame.mouse.get_pos()
		
		framerate.tick(60)
		ticker = pygame.time.get_ticks()

		# Load the map and its layout; create off-screen entrance/exit rects if the map is missing either, to avoid Type errors
		window.blit(map_tuple[map_ID].map_background, (0, 0))
		...
		if pressed_keys[pygame.K_LEFT]:
			while hero.speed > 0:
				...
				test_hero_y = hero.y
				wall_collision_index = pygame.Rect(test_hero_x, test_hero_y, hero.width, hero.height).collidelist(drawn_wall_rects)
				if hero.x - hero.speed >= 0 and wall_collision_index == -1:
					hero.x -= hero.speed
					new_map_data = map_change_check(window, hero, map_ID, drawn_map, drawn_map_entrance_rect, drawn_map_exit_rect, wall_surface, found_items_rects, new_map_data)
		...
		# Update rects based on character positions, check for collisions to start a battle and update the parts of the screen that have changes

		onscreen_chars_rects.append(pygame.Rect(hero.x, hero.y, hero.width, hero.height))
		
		if pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_opponents_rects) != -1 and pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(defeated_rects) == -1:
			battle_op_rect_index = pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_opponents_rects)
		...
		if pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_map_items_rects) != -1 and pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(found_items_rects) == -1:
			wait = True
			...
			while wait == True:
				for event in pygame.event.get():
					if event.type == pygame.QUIT:
						...
						wait = False
				mouse_pos = pygame.mouse.get_pos()
				if len(player_inventory.items) < player_inventory.item_capacity and random_item.item_type != 'Weapon upgrade':
					found_item = text.render('You found a ' + random_item.item_name + ' (power lvl: ' + str(random_item.item_power) + '). Add it to your inventory?', True, WHITE)
					...
					if add_option_rect.collidepoint(mouse_pos[0] - 80, mouse_pos[1] - 170) and pygame.mouse.get_pressed() == (1, 0, 0):
						if random_item.item_type == 'Weapon upgrade' and replace_weapon == True:
							player_inventory.replace_item(random_item, player_inventory.items[0])
						else:
							player_inventory.add_item(random_item)
						found_items_rect_index = pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_map_items_rects)
						found_items_rects.append(pygame.Rect(drawn_map_items_rects[found_items_rect_index]))
						wait = False
					elif discard_option_rect.collidepoint(mouse_pos[0] - 80, mouse_pos[1] - 170) and pygame.mouse.get_pressed() == (1, 0, 0):
						found_items_rect_index = pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_map_items_rects)
						found_items_rects.append(pygame.Rect(drawn_map_items_rects[found_items_rect_index]))
						wait = False
				elif random_item.item_type == 'Weapon upgrade':
					found_item = text.render('You found a ' + random_item.item_name + ' (power lvl: ' + str(random_item.item_power) + '). Use it as your main weapon? Current weapon power lvl: ' + str(weapon_power), True, WHITE)
					found_item_rect = found_item.get_rect(topleft = (50, 40))
					msg_surface.blit(found_item, (50, 40))
					...
					found_item = text.render('You found a ' + random_item.item_name + ' (Power: ' + str(random_item.item_power) + '), but your inventory is full. Replace it with something?', True, WHITE)
					found_item_rect = found_item.get_rect(topleft = (50, 40))
					msg_surface.blit(found_item, (50, 40))
					replace_option1 = text.render('Replace it with ' + player_inventory.items[0].item_name, True, WHITE)
					replace_option1_rect = replace_option.get_rect(topleft = (50, 80))
					...
					replace_option2_rect = replace_option.get_rect(topleft = (50, 120))
					...
					replace_option3_rect = replace_option.get_rect(topleft = (50, 160))
					...
					window.blit(msg_surface, (80, 170))
					pygame.display.update(msg_surface_rect)
					if replace_option1_rect.collidepoint(mouse_pos[0] - 80, mouse_pos[1] - 170) and pygame.mouse.get_pressed() == (1, 0, 0):
						player_inventory.replace_item(random_item, player_inventory.items[0])
						found_items_rect_index = pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_map_items_rects)
						found_items_rects.append(pygame.Rect(drawn_map_items_rects[found_items_rect_index]))
						wait = False
					elif replace_option2_rect.collidepoint(mouse_pos[0] - 80, mouse_pos[1] - 170) and pygame.mouse.get_pressed() == (1, 0, 0):
						player_inventory.replace_item(random_item, player_inventory.items[1])
						found_items_rect_index = pygame.Rect(hero.x, hero.y, hero.width, hero.height).collidelist(drawn_map_items_rects)
						found_items_rects.append(pygame.Rect(drawn_map_items_rects[found_items_rect_index]))
						wait = False
					elif replace_option3_rect.collidepoint(mouse_pos[0] - 80, mouse_pos[1] - 170) and pygame.mouse.get_pressed() == (1, 0, 0):
		...
		new_opponents_rects = []
		# if ticker > old_ticker + 15:
			# for o_rect in drawn_opponents_rects:
				# if o_rect not in defeated_rects:
					# new_o_rect_pos = chase((o_rect[0], o_rect[1]), drawn_wall_rects, hero.x, hero.y)
					# if new_o_rect_pos != [o_rect[0], o_rect[1]]:
						# new_opponents_rects.append(pygame.Rect(int(new_o_rect_pos[0]),int(new_o_rect_pos[1]), char_width, char_height))
			# old_ticker = ticker
		# if new_opponents_rects != []:
